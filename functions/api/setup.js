...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!env.DB) {
      return new Response(JSON.stringify({ error: 'D1 database is not bound to this Pages project (env.DB missing). Check the D1 binding in wrangler.toml / Cloudflare dashboard.' }), { status: 500 });
    }
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: 'Database query failed. Has the migration (0001_init.sql) been run against this database?', detail: String(dbErr) }), { status: 500 });
    }
