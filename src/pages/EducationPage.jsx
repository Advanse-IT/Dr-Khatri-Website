...
export default function EducationPage({ article }) {
  ...
  return <>
    <SvgIcons /><Header />
    <main id="main-content" className="route-main education-route">
      <section className="route-hero education-hero"><div className="wrap route-grid"><div><span className="kicker">Patient Education</span><h1 className="route-title">{isArticle ? article.h1 : 'Heart Health Patient Education'}</h1><p className="route-lead">{isArticle ? article.description : 'Professional, patient-friendly guides about symptoms, cardiac testing, procedures and when to seek urgent help.'}</p><div className="route-actions"><a className="btn btn-gold" href="/services">View Services</a><a className="btn btn-ghost dark" href="/#contact">Book Appointment</a></div></div><aside className="authority-card" aria-label="Education note"><strong>Cardiology information patients can understand</strong><span>Written in a clear, patient-friendly format</span><span>Linked to relevant specialist services</span><span>Includes urgent-care guidance where appropriate</span></aside></div></section>
    ...
    </main><Footer /><MobileBottomBar /><DirectionsPicker /><CookieConsent />
  </>;
}
